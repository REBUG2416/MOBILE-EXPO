...
app.post("/register", async (req, res) => {
  ...
  if (!username || !password) return res.status(400).json({ error: "Missing fields" + req.body })
...
app.post("/login", async (req, res) => {
  ...
  if (!username || !password || !token) return res.status(400).json({ error: "Missing fields" })
...
app.post("/user-by-connectionId", async (req, res) => {
  ...
  try {
    const user = await User.findOne({
      where: sequelize.where(sequelize.fn("SUBSTRING", sequelize.col("connectionid"), 1, 13), connectionId),
    })
